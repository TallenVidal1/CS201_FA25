...
penguins %>%
  dplyr::filter(sex == "FEMALE", culmen_length_mm >=39,island %in% c("Dream", "Torgerson") )
...
count <- penguins %>%
  ...
  count(island, species, sex)                # count by island, species, sex
#ChatGPT Assisted
view(count)
...
final_boss <- penguins %>%
  filter(
    sex == "FEMALE",!is.na(flipper_length_mm),island %in% c("Dream", "Biscoe")
  ) %>%
