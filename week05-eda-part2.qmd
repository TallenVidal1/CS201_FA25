...
-   Box: represents the IQR (Interquartile Range), the top of the box is the 75th percentile, and the bottom of the box is the 25th percentile.

-   Whiskers: Shows the spread of most of the data extends to the highest and lowest data points
...
ggplot(diamonds, aes(x = carat, y = price)) +
  geom_point(alpha = 0.5) # try making the points transparent to 0.05
```
...
-   Where do we see a lot of variability?
    ...
    We see a lot of variability between carats and cut type, there may be a slight negative correlation. Overall there is no co-variation between the two variables of cut type and carats.

-   Is there a relationship between carat and cut?

    There may be a slight negative correlation but besides that, there seems to be little to no co-variation.

-   Which category has the largest carat diamonds?

    The fair cut category has the largest carat diamonds.
