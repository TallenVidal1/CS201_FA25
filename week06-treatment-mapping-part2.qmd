...
Answer: Variable names are different (stfips vs. state_fips), values are different (state map data had "01,02, 03" vs our teds_d data identifying stfips as "1, 2, 3").

```{r}
## Creates new column in a percent_completed_by_state df called state_fips, forcing values to be two digits (start with 0).
...
## Ways to fill in West Virginia using binning ##

## percent_completed_by_state_map <- percent_completed_by_state_map %>% 
 # mutate(percentage_bin = cut(percentage_completed, breaks=c(0, 10, 20, 30, 40, 50, 60, 70, 80), right = FALSE))

 percent_completed_by_state_map <- percent_completed_by_state_map %>% 
  mutate(percentage_bin = cut(percentage_completed, breaks=c(0, 10, 20, 30, 40, 50, 60, 70, 80), include.lowest = TRUE))
...
## Fixing West Virginia inseert (na.value = "pink") after "direction = -1"##
