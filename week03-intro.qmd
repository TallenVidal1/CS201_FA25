...
author: "Amber Camp (Edited by Tallen)"
date: "2025-09-08"
format: html
editor: visual
---
...
## Transforming the data, creating a model column
...
-   qsec (The time it takes to travel a quarter of a mile), vs, drat,
...
Create a scatterplot, histogram, and boxplot of two variables of your choice (but not `mpg` vs. `wt`). Feel free to try a `ggplot2` version if you have some R experience!
...
```{r}
hist(mtcars$qsec,
     xlab= "1/4 Time", ylab = "Frequency",
     main = "Histogram of 1/4 Quarter Mile Time",
     col = blues9)
```

```{r}
boxplot(mtcars$mpg, mtcars$qsec,
        xlab = "MPG", ylab = "1/4 Mile Time",
        Main = "MPG vs 1/4 Time")
