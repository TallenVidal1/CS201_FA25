...
```{r, message=FALSE}
penguin_scatter<-ggplot(data = penguins_male_female, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
  facet_wrap(~sex) + # here is where we designate faceting!
...
2.  Now, create a bar graph using the *new* dataframe you made
    ...
    ```{r}
    penguin_average_mass<-ggplot(data = average_mass, aes(x = species, y = average_body_mass )) +
      #theme_bw()+
      #scale_color_viridis_b()
      geom_bar(stat="identity")
    ggplotly(penguin_average_mass)# fill in the blanks
...
```{r, message=FALSE}
penguin_average_flipper<-ggplot(data = average_flipper_length, aes(x = species, y = average_flipper_length, fill = species )) +
  theme_bw()+
  coord_flip()+
  theme(legend.position = "none")+
  labs(title = "AVerage Flipper Length by Species", x="", y="Length in mm")+
  geom_bar(stat="identity")
ggplotly(penguin_average_flipper)
...
```{r, message=FALSE}
penguins_boxplot<-ggplot(data = penguins_data, aes(x = species, y = body_mass_g, fill= species)) +
  #theme_bw()+
  #scale_color_viridis_b()
  geom_boxplot()
ggplotly(penguins_boxplot)
...
penguin_average_culmen<-ggplot(data = average_culmen_depth, aes(x = species, y = average_culmen_depth, fill = species )) +
  #theme_bw()+
  #scale_color_viridis_b()
  geom_bar(stat="identity")
ggplotly(penguin_average_culmen)
